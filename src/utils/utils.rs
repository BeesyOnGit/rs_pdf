...
pub fn convert_to_pdf(base_64: String) -> Result<Vec<u8>, String> {
    // importing the path to the chromium headless_shell binary
    // let path = match PathBuf::from_str("/home/rs_pdf/build/headless-shell/headless_shell") {
    let path = match PathBuf::from_str("/home/rs_pdf/cr/chrome-linux/chrome") {
        Ok(path) => path,
        Err(err) => {
    ...
    // let browser_config = match LaunchOptions::default() {
    //     Ok(conf) => conf,
    //     Err(err) => {
    //         println!("error occured : {err}");
    //         return Err(format!("{err}"));
    ...
    let _ = match tab.navigate_to(&base_64) {
        Ok(_b) => _b,
        Err(err) => {
            println!("error occured : {err}");
            return Err(format!("{err}"));
    ...
    let pdf_data = match tab.print_to_pdf(Some(print_options)) {
        Ok(pdf) => pdf,
        Err(err) => {
            println!("error occured : {err}");
            return Err(format!("{err}"));
...
pub fn convert_mm_to_in(mm: i32) -> f64 {
